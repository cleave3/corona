const covid19ImpactEstimator = (data) => {
  ...
  const {
    ...
    avgDailyIncomePopulation: avgPop
  } = region;
  ...
  const severeSevereCasesByRequestedTime = Math.trunc(
  ...
  const sHospitalBedsByRequestedTime = Math.trunc(
    hospitalBeds - severeSevereCasesByRequestedTime
  );
  ...
  const $sInFlight = Math.trunc(
    (infectionsByRequestedTime * avgIncome * avgPop) / days
  );
  const s$sInFlight = Math.trunc(
    (severeInfectionsByRequestedTime * avgIncome * avgPop) / days
  );
